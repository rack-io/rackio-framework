...
class QueryLogger:
    ...
    def query_waveform(self, tag, start, stop, sampling_time=SAMPLING_TIME):
        ...
        values = _query.where(
            (TagValue.timestamp > start) & (TagValue.timestamp < stop)
        )
        ...
        values = [
            value.value for count, value in enumerate(values) if count % sampling == 0
        ]
    ...
    def query_trend(self, tag, start, stop, sampling_time=SAMPLING_TIME):
        ...
        values = [
            {"x": value.timestamp.strftime(OUTPUT_DATETIME_FORMAT), "y": value.value}
    ...
    def query_last(
        self, tag, seconds=None, values=None, waveform=False, sampling_time=1
    ):
        if seconds:
            stop = datetime.now()
            ...
            if waveform:
                return self.query_waveform(
                    tag, start, stop, sampling_time=sampling_time
                )
